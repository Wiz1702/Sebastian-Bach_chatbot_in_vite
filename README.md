...
- Session continuity: the frontend stores a `bach-chat-session-id` in `localStorage` and sends it as an `X-Session-ID` header so the Durable Object can keep a conversation thread even when the UI runs on a different origin during local development. In production (same origin), cookies continue to work automatically.
